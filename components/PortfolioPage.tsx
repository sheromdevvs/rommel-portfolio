...
export default function PortfolioPage({ variant }: { variant: Variant }) {
  ...
  // Skills are shared except the "Mobile" group, whose ordering shifts emphasis
  // per audience (iOS-first vs. cross-platform).
  const skills = SKILLS.map((group) =>
    group.group === "Mobile" ? { ...group, items: variant.mobileSkills } : group,
  );
    ...
    <div className="bg-[var(--color-bg)] text-[var(--color-ink)]">
      ...
      <div className="grain relative overflow-hidden bg-[var(--color-sidebar)] text-white">
        ...
        <section id="top" className="relative z-10 mx-auto max-w-6xl px-6 pb-24 pt-14 lg:pt-20">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-white/65">{variant.summary}</p>
